...
<head>
    ...
    <link href="{{ url ('assets/dashboard/vendor/css-hamburg$clocks/hamburg$clocks.min.css') }}" rel="stylesheet" media="all">
...
<body class="animsition page-wrapper">
    ...
    <div class="page-content">
        ...
        <section class="welcome p-t-10">
            <h1 class="text-center">Registro de atrasos</h1>
            ...
            <div class="col-md-12">
                ...
                <div class="col-md-6 mx-auto">
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <form action="{{ route('clock.index') }}">
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            <label for="name" class="control-label mb-1">Nome</label>
                                            <input id="name" name="name" class="form-control" placeholder="Digite o nome do funcionário">
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <label for="adjuntancy" class="control-label mb-1">Cargo</label>
                                        <div class="input-group">
                                            <input id="adjuntancy" name="adjuntancy" class="form-control" placeholder="Digite o cargo do funcionário">
                                        </div>
                                    </div>
                                </div>
                                <div class="text-center">
                                    ...
                                    <a class="btn btn-warning btn-sm text-dark" href="{{ route('clock.index') }}">
                                        <i class="fa-solid fa-chevron-left"></i> Voltar
                                    </a>
                                    <button type="reset" class="btn btn-danger btn-sm text-dark">
                ...
                <div class="card mt-4 mb-4 border-light shadow">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <span>
                            <button type="button">
                                <a href="{{ route('clock.create') }}" class="btn btn-success text-dark">
                                    <i class="fa-solid fa-plus"></i> Cadastrar
                                </a>
                            </button>

                            <button type="button">
                                <a href="{{ route('pdf.generate', request()->query()) }}" class="btn btn-info text-dark mr-2">
                    ...
                    <div class="card-body">
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($Clock as $clocks)
                                    <tr>
                                        <th>{{ $clocks->code }}</th>
                                        <td>{{ $clocks->name }}</td>
                                        <td>{{ $clocks->adjuntancy }}</td>
                                        <td>{{ $clocks->admission }}</td>
                                        <td>{{ $clocks->contract1 }}</td>
                                        <td>{{ $clocks->contract2 }}</td>
                                        <td>{{ 'R$ ' . number_format($clocks->salary, 2, ',', '.') }}</td>

                                        <td class="d-md-flex justify-content-center">

                                            <form action="{{ route('clock.edit', $clock->code) }}" method="GET" class="mr-2">
                                                @csrf
                                            ...
                                            <form action="{{ route('clock.destroy', $clock->code) }}" method="POST">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" class="btn btn-danger text-dark"
                                                    onclick="return confirm('Tem certeza que deseja apagar este registro?')">
                                                    <i class="fa-solid fa-trash"></i> Apagar</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
